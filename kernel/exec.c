...
kexec(char *path, char **argv)
{
  ...
  if(stack_pa)
    printf("USER STACK created for pid %d name %s at va=0x%lx pa=0x%lx\n", p->pid, p->name, stack_va, stack_pa);
  else
    printf("USER STACK created for pid %d name %s at va=0x%lx pa=UNKNOWN\n", p->pid, p->name, stack_va);
  if(p->pid <= 2) {  // Only log for first two processes to avoid noise
    printf("USER STACK: dynamically allocated via uvmalloc() - same VA (0x%lx) across processes\n", stack_va);
    printf("USER STACK: different PA per process ensures isolation (freed on exit)\n");
    printf("USER STACK: grows downward from sp=0x%lx (RISC-V convention)\n", sz);
  }
